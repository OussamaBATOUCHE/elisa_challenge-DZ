...
def minimize(dataset, cost_func=cost.score_loss_acc, bounds=[(1, 5), (10, 512), (21, 301), (0.001, 0.1)], popsize=5, mutate=0.5, recombination=0.7, maxiter=20):
    ...
    for i in range(1, maxiter+1):
        ...
        for j in range(0, popsize):
            ...
            model_diff = [model_2_i - model_3_i for model_2_i,
                          model_3_i in zip(model_2, model_3)]

            # multiply model_diff by the mutation factor (F) and add to model_1
            v_donor = [model_1_i + mutate * model_diff_i for model_1_i,
                       model_diff_i in zip(model_1, model_diff)]
        ...
        gen_sol = population[gen_scores.index(gen_best)]
        ...
        # add the best soluction in this generation
        all_gen_best.append(gen_sol)
        # add the best score in this generation
        all_gen_best_score.append(gen_best)
    ...
    for k in range(len(all_gen_best)):
        print('       - [ ', all_gen_best_score[k],
              ' : ', all_gen_best[k], ' ]')

    # Selection the best of the best
    best_gens_sol = all_gen_best[all_gen_best_score.index(
        min(all_gen_best_score))]
    print("  RESULTS ")
    print("  -------- ")
