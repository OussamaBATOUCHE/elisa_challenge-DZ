    ...
    for i in range(nb_layers):
        if i == 0:
            model.add(layers.Dense(
                nb_percep, activation=activationFunctions[0], input_dim=3))
        else:
            model.add(layers.Dense(
                nb_percep, activation=activationFunctions[0]))
...
def train(model_architecture, dataset):
    ...
    logfilename = str(nb_layer)+"_"+str(nb_perceptron)+"_"+str(nb_epoch) + \
        "_"+str(l_rate)[2:]+"_"+str(datetime.datetime.now().strftime("%H%M%S"))
